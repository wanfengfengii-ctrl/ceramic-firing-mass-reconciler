...
另有一个**完全独立的日常秤检台账**：开工前用标准砝码确认电子秤示值，检验结果不落在纸单、
也**不参与、不阻断**任何批次核算（见下文「日常秤检台账」）。
...
窑边电子秤**每日开工前**用标准砝码确认示值可靠。工艺员在「日常秤检」工作台填写
**设备编号、检验日期和三组标准重量/实测重量**：

- 页面即时显示每组**带符号偏差**（偏差 = 实测 − 标准，固定三位小数，正为示值偏大、负为偏小）；
- 提交后按**任一测点偏差绝对值不超过 0.500 克**（即三组都不超差）判定**合格/不合格**并展示本次结论；
- 每次秤检以**设备编号 + 检验日期唯一**保存：标准值、实测值、带符号偏差与结论都写入
  独立的 `scale_checks` 表；刷新工作台后通过同一资源（`GET /api/scale-checks`）
  恢复**按检验日期倒序**的记录；
- 计算全程十进制定点：后端 `decimal.Decimal`，前端把克重放大为「毫克」整数（BigInt），
  二进制浮点不参与任何裁决。

合法但**不合格**的秤检（存在超差点）照常保存，结论显示「不合格」。
设备编号为空、日期不是真实 `YYYY-MM-DD`、重量非正/超过三位小数/非十进制、测点数不是三组
都返回 **400** 并在页面上把反馈定位到**具体测点的标准/实测字段**；同设备同日期重复检验
返回 **409** 并指出重复日期。失败请求整体回滚，**不留下部分记录**，页面当前输入保留。

秤检与批次核算互不引用：秤检不合格不会阻断批次创建、导入、详情或对比，批次数据也不读秤检结果。
...
- `POST /api/scale-checks` — 保存一次秤检（独立于批次，201 返回含复算偏差与结论）
  ```json
  {
    "device_no": "DC-03",
  ...
  ```
  重量一律为正的三位小数字符串（不接受 JSON 数字）；后端用 Decimal 复算
  `deviation = measured − standard`（带符号三位小数），三组 `|deviation| ≤ 0.500`
  才 `passed: true`。非法输入 400 且不留记录；同设备同日期重复 409。
- `GET /api/scale-checks` — 秤检台账，按检验日期倒序（同日按保存先后倒序）返回；
  每条含设备、日期、三组标准/实测/偏差与结论。刷新工作台即通过该查询契约恢复。
