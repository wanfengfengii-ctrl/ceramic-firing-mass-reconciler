...
未知分区、重复表头、字段矛盾或任一重量/份数非法时**整份拒绝**，返回 400 与
`{"detail", "line", "reason"}`（`line` 为 CSV 行号，文件级错误为 `null`）；
预检只读不写库，非法文件零落库。
...
- `POST /api/batches/import-preview` — 称重文件导入预检（**不写数据库**）
  ...
  允许差、裁决）。空行忽略；未知分区、重复表头、字段矛盾或任一重量非法时整份拒绝，
  返回 400 与 `{"detail", "line", "reason"}`（`line` 为原始 CSV 行号，文件级错误为
